...
for (var i = 0; i < document.querySelectorAll("button.drum").length; i++) {
    document.querySelectorAll("button")[i].addEventListener("click", function () {
        ...
        makesound(letter);
        addButtonAnimation(letter);
    }
    );
}
// For keyboard press
document.addEventListener("keydown", function (event) {  // it set for all the documents
    makesound(event.key);
...
function addButtonAnimation(key) {
    ...
    setTimeout(function () {            // setTimeout(function, millisecond);
